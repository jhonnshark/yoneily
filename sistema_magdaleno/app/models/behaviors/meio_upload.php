...
class MeioUploadBehavior extends ModelBehavior {
	...
	var $default_options = array(
		...
		'validations' => array(
			...
			'Dir' => array(
				...
				'check' => true,
				'message' => 'El directorio del archivo no existe o esta protegido por contraseña.'
			),
			'Empty' => array(
			...
			'MaxSize' => array(
				...
				'check' => true,
				'message' => 'El tamaño no es el permitido.'
			),
			'InvalidMime' => array(
	...
	var $default_validations = array(
		...
		'Empty' => array(
			...
			'check' => true,
			'on' => 'create',
			'message' => 'El campo archivo no puede estar vacio'
		),
		'UploadError' => array(
			'rule' => array('uploadCheckUploadError'),
			'check' => true,
		...
		'InvalidMime' => array(
			'rule' => array('uploadCheckInvalidMime'),
			'check' => true,
			'message' => 'Tipo de archivos invalidos.'
		),
		'InvalidExt' => array(
			'rule' => array('uploadCheckInvalidExt'),
			'check' => true,
	...
	function setup(&$model, $config=array()) {
		...
		$this->__model = $model;
//		$pruebamodelo = $this->__model;
//                var_dump($pruebamodelo);
                $this->__fields = array();
                //pr($config);
	...
	function replaceTokens($string,$fieldName){
                ...
                $pruebastr = str_replace(array('{model}', '{field}', '{DS}','/','\\'),array(Inflector::underscore($this->__model->name),$fieldName,DS,DS,DS),$string);
	...
	function beforeSave(&$model) {
		...
		foreach($this->__fields as $fieldName=>$options){
			...
			if(!empty($model->data[$model->name][$fieldName]['remove'])){
				//pr($model->data[$model->name][$fieldName]['remove']);
                                if($options['default']){
